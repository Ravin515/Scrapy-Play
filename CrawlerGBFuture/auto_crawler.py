...
while True:
    ...
    if not isRunning:                       # 爬虫不在执行，开始启动爬虫
        ...
        print(f"GBFuture not running, ready to start. isExsit:{isExsit}")
        if isExsit:
        ...
        print(f"At time:{clawerTime}, start clawer: GBFuture !!!, waitTime:{waitTime}")
        cmdline.execute('scrapy crawl GBFuture -s JOBDIR=crawls/storeMyRequest'.split())
        break  #爬虫结束之后，退出脚本
    else:
        print(f"At time:{datetime.datetime.now()}, GBFuture is running, sleep to wait.")
    i += 1
