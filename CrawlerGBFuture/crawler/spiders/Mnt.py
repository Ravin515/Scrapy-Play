...
class GBFuture(Spider):
    ...
    def parse(self, response):
        ...
        for path in paths:
            ...
            yield Request(url = url, callback = self.parse_page, meta = {'page': 1, 'semi_url': copy.deepcopy(semi_url), 'item': copy.deepcopy(item)})
    ...
    def parse_page(self, response): # 确定需要抓取的页数
        ...
        if md in tag_date:
            yield Request(url = 'http://guba.eastmoney.com/'+ semi_url + 'f_' + str(page + 1) + '.html', callback = self.parse_page, dont_filter = True, meta = {'page': page + 1, 'item' : item, 'semi_url': semi_url})
        
        elif md not in tag_date:
            for i in range(1, page):
                yield Request(url = 'http://guba.eastmoney.com/'+ semi_url + 'f_' + str(i) + '.html', callback = self.parse_post, dont_filter = True, meta = {'item' : item, 'md': md})
